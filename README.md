...
CLI client for streaming camera frames to Databricks ingestion tables via the Zerobus SDK. Supports RTSP streams (via
FFmpeg), Meraki camera snapshots, and local image directories for testing/replay.
...
- `DATABRICKS_ZEROBUS_IP_RESOLVE`: Set to `true` to skip local DNS resolution and resolve the Zerobus endpoint using
  public DNS (`1.1.1.1`).
